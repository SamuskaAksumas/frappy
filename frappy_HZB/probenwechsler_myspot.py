...
class Magazin:
    ...
    def mag2Arrayof(self):
        ...
        for sample in  self.Mag:
            if sample:
                storage_arr.append(sample.sample_name())
...
class Sample(HasIO,Drivable):
    ...
    Status = Enum(Drivable.Status,
                  ...
                  MEASURING_SAMPLE = 101, # check if correct or needs to be changed 
                  MOUNTING=301,
                  UNMOUNTING = 302,
                  UNLOADING = 304,
                  MEASURING = 303,
    ...
    value = Parameter("Active Sample currently in measurement position (0 == no sample)",
                      datatype=IntRange(minval=0,maxval=nsamples),
                      readonly = True,
                      default = 0) 

    target =  Parameter("Target Sample to be put to measurement position",
                      datatype=IntRange(minval=0,maxval=nsamples),
                      default = 0)
    ...
    sample_name = Parameter("Name of Sample currently in measurement position",
                            datatype=StringType(),
                            default = 'none',
                            group = 'sample',
                            readonly = True)
    ...
    #                  readonly= True,
    #                  group = 'sample')
    sample_id = Parameter("ID assigned to sample currently in measurement position",
                     datatype=StringType(),
                     default = 'none',
                     readonly= True,
                     group = 'sample')
    ...
    def read_status(self):
        ...
        if robo_stat[0] == IDLE and self._measuring_sample():
            
            return MEASURING_SAMPLE , "IDLE with Sample in measurement position"
        
        # Robot Arm is Busy        
        if robo_stat[0] == BUSY:
            # if re.match(r'messpos\d+\.urp',self.attached_robot.value):
            #     return  MOUNTING, "Mounting Sample"
            if re.match(r'messin\d+\.urp',self.attached_robot.value):
                return MOUNTING , "Mounting Sample"
    ...
    def _mount(self,target):
        """Mount Sample to measurement psoition"""
        assert(target != 0)
        ...
        # check if a sample is in measurement position
        if self._measuring_sample():
            raise ImpossibleError('Sample already in measurement position' + str(self.value))       
     
        # Run Robot script to mount actual Sample        
        prog_name = 'messin'+ str(target) + '.urp'
    ...
    def _unmount(self,target):
        """Unmount Sample from measurement position to magazine"""
        ...
        if not self._measuring_sample():
            raise ImpossibleError('No sample in measurement position, cannot unmount nothing')   
        
        
        # Run Robot script to unmount Sample        
        prog_name = 'messout'+ str(self.value) + '.urp'
...
class Storage(HasIO,Readable):
    ...
    def read_status(self):
        ...
        if self.attached_sample._measuring_sample():
            return BUSY , "Robot is in use by another module"
    ...
    def unload(self,sample_pos):
        ...
        # check if robot is ready to load sample
        if self.attached_sample._measuring_sample() == True:
            raise ImpossibleError('Sample already in measuring position' + str(self.attached_sample.value)+" try unloading via 'sample' module")
...
MEASURING_SAMPLE   = Sample.Status.MEASURING_SAMPLE
